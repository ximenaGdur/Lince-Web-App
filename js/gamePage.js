...
class GamePage {
  ...
  static firstCard = null;

  // Current player score
  static score = 0;

  // Interval that changes time each second.
  static secondInterval = null;
   ...
   */
  stopGame(socket) {
    if (this.secondInterval && playerNickname && roomCode) {
  ...
  createContentElement(imageText, route, className) {
  ...
  match(socket, secondCard) {
    if (this.firstCard && playerNickname && roomCode) {
      const message = {
        type: 'checkMatch',
        from: 'player',
        to: 'server',
      ...
      Object.keys(boardCards).forEach((cardId) => {
        ...
        if (cardData) {
          ...
          if (this.configMap) {
            ...
            const contentElement = this.createContentElement(imageText, cardData.route, 'board-image');
            if (contentElement) {
      ...
      Object.keys(cardsReceived).forEach((cardId) => {
        ...
        if (cardData) {
          ...
          if (this.configMap) {
            ...
            const contentElement = this.createContentElement(imageText, cardData.route, 'my-image');
            if (contentElement) {
              // If player has selected colored border
              if (this.configMap.adaptation2a === true || this.configMap.adaptation2b === true) {
                cardElement.style.borderColor = cardData.border;
